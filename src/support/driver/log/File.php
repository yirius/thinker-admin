...
namespace Yirius\Admin\support\driver\log;
...
class File
{
    ...
    protected function getMasterLogFile()
    {
        ...
        if ($this->config['single']) {
            ...
            $destination = $this->config['path'] . $name . '_' . str_replace(".", "_", Request::server("SERVER_ADDR")) . '.log';
        } else {
            $cli = PHP_SAPI == 'cli' ? '_cli' : '';

            if ($this->config['max_files']) {
                $filename = date('Ymd') . $cli . '_' . $this->getServerAddr() . '.log';
            } else {
                $filename = date('Ym') . DIRECTORY_SEPARATOR . date('d') . $cli . '_' . $this->getServerAddr() . '.log';
            }
    ...
    protected function getServerAddr()
    {
        if(PHP_SAPI == 'cli'){
            try{
                return str_replace(".", "_", gethostbyname(gethostname()));
            }catch (\Exception $exception){
                return '127_0_0_1';
            }
        }else{
            return str_replace(".", "_", Request::server("SERVER_ADDR"));
        }
    }
    ...
    protected function getApartLevelFile($path, $type)
    {
        ...
        return $path . DIRECTORY_SEPARATOR . $name . '_' . str_replace(".", "_", Request::server("SERVER_ADDR")) . '.log';
    }
    ...
    protected function parseLog($info)
    {
        ...
        $noStack = config("thinkeradmin.log.nostack");
        if(empty($noStack)) $noStack = ['info'];
        foreach($noStack as $item){
            if(isset($info[$item])){
                unset($requestInfo['stack']);
                break;
            }
        }
